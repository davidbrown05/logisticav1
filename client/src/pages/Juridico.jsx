...
import { TabsJuridico } from "../components/TabsJuridico";

import UsuarioProvider from "../context/UsuarioContext";

export const Juridico = () => {
  const { currentUser } = useSelector((state) => state.user);
  console.log("juridicoCurrentUser", currentUser);
  ...
  const [isLoadingJuridico, setisLoadingJuridico] = useState(true);
  const [juridicoObjectString, setJuridicoObjectString] = useState(null);
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center m-10">
        <img src={foto} className="h-[100px] w-[100px] md:w-[150px] md:h-[150px] lg:w-[180px] lg:h-[180px] rounded-lg" />
        <div className=" flex flex-col  items-center  justify-between ">
          <h1 className="text-[20px] md:text-[30px] text-center p-8 font-semibold">
            MODULO JURIDICO:
          </h1>

          <p className=" font-semibold text-center text-[15px] md:text-[20px] xl:text-[20px] text-gray-500">{direccion}</p>
        </div>
      </div>
      {isLoadingJuridico ? ( 
        <UsuarioProvider id={currentUser._id}>
          <JuridicoProvider id={id}>
            <TabsJuridico id={id} />
            <PropertyMenu
        ...
        <div className="flex flex-col items-center justify-center">
          <p>Cargando datos de jurídico...</p>
