...
export const PagosFinalesModal = ({
  openPagosFInalModal,
  setopenPagoasFinalModal,
  pagoData,
  pagoIndex,
  partnerInfo,
  setpartnerInfo,
  ...
  const [abonosFinales, setAbonos] = useState(
    Array.isArray(pagoData.abonos) ? pagoData.abonos : []
  );
  ...
  useEffect(() => {
    if (pagoData && pagoData.status) {
      ...
      setAbonos(pagoData.abonos); // Actualiza selectedOption cuando pagoData esté disponible
    }
  }, [pagoData]);
      ...
      setpartnerInfo({
        ...
        pagosFinales: partnerInfo.pagosFinales.map((pagoFinal, pagoInd) => {
          if (pagoInd === pagoIndex) {
            return {
              ...pagoFinal,
              abonos: nuevosAbonos, // Aquí actualizas el campo 'abonos' dentro de 'pagosFinales'
            };
          }
          return pagoFinal;
        }),
        ...
        setpartnerInfo({
          ...
          pagosFinales: partnerInfo.pagosFinales.map((pagoFinal, pagoInd) => {
            if (pagoInd === pagoIndex) {
              return {
                ...pagoFinal,
                abonos: nuevosAbonos, // Aquí actualizas el campo 'abonos' en pagosFinales
              };
            }
            return pagoFinal;
          }),
  ...
  return (
    <>
      {openPagosFInalModal && (
        <div
          onClick={() => setopenPagoasFinalModal(!openPagosFInalModal)}
            ...
            <div className="w-full p-5 flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div>
                  ...
                  <p>${pagoData.pagoTotal.toLocaleString("es-MX")}</p>
                </div>
                <div>
              ...
              <div className="flex flex-col items-start justify-between gap-5">
                ...
                {/* Si quieres agregar un botón o input adicional para cancelar */}
                <div className="flex gap-3">
                  <input
                    type="radio"
                    value="cancelado"
                    checked={selectedOption === "cancelado"}
                    onChange={(e) => setSelectedOption(e.target.value)} // Cambiar a cancelado
                  />
